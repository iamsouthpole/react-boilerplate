...
export default class TimeLimit extends React.Component {
  ...
  componentDidMount() {
    const time = window.localStorage.getItem('time_create') || undefined;
    if (time) {
      this.intervalHandle = setInterval(this.tick, 1000);
      const oneSec = 1000;
      const diff = Math.round(Math.abs((time - new Date().getTime()) / oneSec));
      this.secondsRemaining = 10 * 60 - diff;
      this.setState({
        isClicked: true,
      });
    }
